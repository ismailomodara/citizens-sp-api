...
export async function index(): Promise<ApiResponse<Service[]>> {
  try {
    const result = await query<Service>('SELECT label, code, description FROM services WHERE status_id = $1 ORDER BY id', [Statuses.ENABLED]);
    // Note - if Admin, return all services (enabled or disabled)
    // const result = await query<Service>('SELECT label, code, description FROM services ORDER BY id');
    return {
      success: true,
      data: result.rows,
